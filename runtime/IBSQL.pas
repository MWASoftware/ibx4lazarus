...
{ IBSQL param names can be in dialect 3 quoted format (e.g. :"MyParam").
 ...
 descendents. However, they are used internally by IBX to simply SQL generation
 for TTable components in Master/Slave relationships which are linked by
 Dialect 3 names. They are also generated by TStoredProc when the original
 parameter names are quoted. By default IBX continues to support Dialect 3
 quoted names.

 However, for some users they do cause a big processing overhead -and which is
 not justifiable when the TTable/TStoredProc cases are known not to occur.
 The code to support quoted parameter names is now subject  to conditional compilation.
 To disable support, ALLOWDIALECT3PARAMNAMES should be undefined when IBX is compiled.

 Hint: add a space between the brace and the dollar sign below

 Note that disabling Dialect 3 quoted format parameter names may break TTable or TStoredProc.
}
