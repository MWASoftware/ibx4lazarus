...
This example application illustrates use of the TIBXScript SQL script engine. It works with the example employee database and with various test scripts to illustrate how it works. These are all located in the "tests" directory.
...
You can just type SQL queries into the left hand text box and click on "Execute" to run them. The results appear in the right hand text box. Note that Select Queries are not supported and will result in an error message.

The test scripts are loaded in the left hand text box bby clicking on the "Load Script" button. The scripts are:

1. CreateCountriesTable.sql

This adds a new table "COUNTRIES" to the employee database and then populates it with country data including the country name and ISO2 and 3 character shortnames.
...
This script demonstrates the use of IBX style query parameters for BLOB columns. In this case a new column "Image" is added to the COUNTRY Table and an image in png format (the flag of St George) is added to the entry for England. The value of the Image column is given by a parameter ":MyImage". This is resolved by the application which asks for the file containing the image to be placed in the field. You should locate and return the "flag_en.png" file.
